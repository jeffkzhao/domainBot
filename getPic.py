...
def run_experiments(isn1=True, isn2=True, isn3=True, issigmoid=True,  nfolds=10):
    ...
    return n1_results, n2_results, n3_results, sigmoid_results

def create_figs(isn1=True, isn2=True, isn3=True, issigmoid=True, nfolds=10, force=False):
    """Create figures"""
    # Generate results if needed

    results = pickle.load(open(RESULT_FILE))
    results1 = pickle.load(open('results1.pkl'))

    # Extract and calculate bigram ROC
    if results['n1']:
        n1_results = results1['n1']
        fpr = []
        tpr = []
    ...
    with plt.style.context('bmh'):
        ...
        if issigmoid:
            plt.plot(sigmoid_binary_fpr, sigmoid_binary_tpr,
                    label='Truong (AUC = %.4f)' % (sigmoid_binary_auc,), rasterized=True)
...
if __name__ == "__main__":
    #create_figs(isn1=False, isn3=False, issigmoid=False, nfolds=1) # Run with 1 to make it fast
